...
data1 <- data0 %>% 
  select(2,3,5,6,8,9,10,11,12,13)

data1.log <- data1 %>% 
  mutate(Lg_fertilizer=log(Fertilizer_use), Lg_gdp=log(GDP_capita), 
...
m1.fixed <- plm(Lg_fertilizer~Lg_gdp+I(Lg_gdp^2)+Lg_area+Lg_export+Lg_rural+Employ_per, 
...
write.csv(Asia.table, file="fert-asia_final.csv")
